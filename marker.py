...
def _smooth_tracks(
    ...
    """对可见帧的轨迹做高斯时序平滑，抑制帧间抖动。

    只在可见帧序列内平滑（不跨越遮挡段），丢失帧位置保持不变。
    sigma 控制平滑窗口大小（帧数标准差），越大越平滑，默认 1.0。
    """
    ...
    # 用 NaN 标记不可见帧，然后仅对可见帧做插值平滑
    # 简单做法：对整条轨迹平滑，丢失帧的位置已经是传播值，影响有限
    for axis in range(2):
        smoothed[visible, axis] = gaussian_filter1d(
            tracks[visible, axis].astype(np.float64), sigma=sigma
        ).astype(np.float32)
    return smoothed
...
def track_marker_sequence(
    ...
    重新检测到后恢复默认搜索半径。最终对可见帧轨迹做高斯时序平滑。
    ...
    # 时序平滑：抑制扩散模型逐帧生成引入的高频抖动
    if smooth_sigma > 0:
        tracks = _smooth_tracks(tracks, visible, sigma=smooth_sigma)
